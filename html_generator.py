...
def handle_post_image(yaml_content, markdown_content, image_dir):
  ...
  if image_url and not image_url.startswith('/images/'):
    ...
    markdown_content = markdown_content.replace(image_url,
                                                '/' + local_image_path)

    # Update the 'image' attribute in the yaml_content
    yaml_content[
        'image'] = '/' + local_image_path  # Update the image path to local path
...
def read_markdown_files(folder_path):
  ...
  for filename in os.listdir(folder_path):
    if filename.endswith(".md"):
      ...
      if new_image_url and new_image_url != original_image_url:
        relative_image_path = os.path.join('/images',
                                           os.path.basename(new_image_url))
  ...
  posts.sort(key=lambda x: x['date']
             if x['date'] is not None else datetime.min,
             reverse=True)
  return posts
